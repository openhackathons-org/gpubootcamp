...
def run_everything():
    ...
    # #### 2.1.3 Series
    #
    # cuDF DataFrames are composed of rows and columns. Each column is represented using an object of type `Series`. For example, if we subset a cuDF DataFrame using just one column we will be returned an object of type `pd.dataframe.series.Series`.
    ...
    # The data type of the 'Hour' column is `object` which means a string. Let's convert this to a numeric value! This cannot be done with the `astype` method, you should use the [pd.to_datetime](https://docs.rapids.ai/api/pd/nightly/api.html#pd.to_datetime) function!
    ...
    weather['Temperature'] = weather['Temperature'].apply(scale_temp)
    ...
    weather['Wind'] = weather['Wind'].apply(scale_wind)
    ...
    weather = weather.drop('RTemp', axis=1)
    ...
    bikes_raw['Start date'] = pd.to_datetime(bikes_raw['Start date'])
    ...
    bikes_raw['Hour'] = bikes_raw['Start date'].dt.floor('h')


    # We will aggregate the number of bicicle rental events for each hour. We use the [groupby](https://docs.rapids.ai/api/pd/nightly/api.html#groupby) function.
    ...
    bikes['date'] = bikes.index.floor('D')


    # It will be usefull to define a set of additional features: hour of the day, day of month, month and year https://docs.rapids.ai/api/pd/nightly/api.html#datetimeindex
    ...
    #weather['Hour'] = weather['Hour'].dt.tz_localize('UTC')
    bikes= bikes.tz_localize('UTC')
    ...
    # We can see that the data is not sorted after the merge use the [sort_values](https://docs.rapids.ai/api/pd/nightly/api.html#pd.core.dataframe.DataFrame.sort_values) method to
    ...
    # We first
    # We can use the [weekday](https://docs.rapids.ai/api/pd/nightly/api.html#pd.core.series.DatetimeProperties.weekday) attribute of the [datetime](https://docs.rapids.ai/api/pd/nightly/api.html#datetimeindex)
    ...
    holidays = pd.DataFrame({'date': ['2011-01-17', '2011-02-21', '2011-04-15', '2011-05-30', '2011-07-04', '2011-09-05', '2011-11-11', '2011-11-24', '2011-12-26', '2012-01-02', '2012-01-16', '2012-02-20', '2012-04-16', '2012-05-28', '2012-07-04', '2012-09-03', '2012-11-12', '2012-11-22', '2012-12-25'],
    ...
    holidays = holidays.drop('Description', axis=1)
    holidays['Holiday'] = 1
    #holidays
    ...
    # More on user defined functions in our [blog](https://medium.com/rapids-ai/user-defined-functions-in-rapids-pd-2d7c3fc2728d) and in the [documentation](https://docs.rapids.ai/api/pd/nightly/guide-to-udfs.html).
    #
    ...
    # "
    #
    # We start by one-hot encoding the 'Weather' column using the [one_hot_encoding](https://docs.rapids.ai/api/pd/nightly/api.html#pd.core.dataframe.DataFrame.one_hot_encoding) method from cuDF DataFrame. This is very the [get_dummies](https://docs.rapids.ai/api/pd/nightly/api.html#pd.core.reshape.get_dummies) function (which might be more familiar for Pandas users), but one_hot_encoding works on a single input column and performs the operation in place.

    # In[81]:

    gdf['hr'] = gdf['hr'].astype('category')
    gdf['month'] = gdf['month'].astype('category')
    gdf['Weather'] = gdf['Weather'].astype('category')
    gdf['Weekday'] = gdf['Weekday'].astype('category')

    gdf = pd.get_dummies(gdf, drop_first=True)
    ...
    y = gdf['cnt'].to_numpy()
    X = gdf.drop('cnt', axis=1).to_numpy()
    ...
    t_stop = timer()
    t_pd = t_pd_stop - t_start
    t_cuml = t_stop - t_cuml_start - t_file_cuml
    return t_pd, t_cuml, t_file, t_file_cuml
...
# benchmark
n_loop = 5
t_df = np.zeros(n_loop)
t_ml = np.zeros(n_loop)
t_file = np.zeros(n_loop)
t_file_ml = np.zeros(n_loop)

for i in range(5):
    t_df[i], t_ml[i], t_file[i], t_file_ml[i] = run_everything()
    print('t_file', t_file[i], 't_df', t_df[i], "t_ml:", t_ml[i], "t_ml_file", t_file_ml[i])

print("Summary (mean and std):")
print("t_file", t_file.mean(), t_file.std())
print("t_df", t_df.mean(), t_df.std())
print("t_ml", t_ml.mean(), t_ml.std())
