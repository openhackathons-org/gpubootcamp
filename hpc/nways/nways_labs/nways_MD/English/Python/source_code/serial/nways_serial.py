...
def dcdreadframe2(ax, ay, az, infile, numatm, i):
    data = infile.readframes(i, i+1)
    box = data[1]
    atomset = data[0][0]
    xbox = round(box[0][0], 8)
    ...
    for row in range(numatm):
        ax[row] = round(atomset[row][0], 8) # 0 is column
        ay[row] = round(atomset[row][1], 8)  # 1 is column
        az[row] = round(atomset[row][2], 8)  # 2 is column

    return xbox, ybox, zbox, ax, ay, az
